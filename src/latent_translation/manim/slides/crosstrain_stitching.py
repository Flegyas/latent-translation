...
def build_stitching_ae(model_id: str, color: str, label_pos):
    ...
    encoder = (
        Polygon(
            2 * UP + LEFT, RIGHT + UP * rr_size, RIGHT + DOWN * rr_size, 2 * DOWN + LEFT, fill_opacity=0.5, color=color
        )
...
class Translation(Scene):
    def construct(self):
        ...
        ae1 = build_stitching_ae(model_id="1", color=encoder_color, label_pos=UP).scale(0.85)
        ae2 = build_stitching_ae(model_id="2", color=decoder_color, label_pos=DOWN).scale(0.85)
